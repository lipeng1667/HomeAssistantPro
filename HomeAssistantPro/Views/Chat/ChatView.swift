...
struct ChatView: View {
    ...
    /// Computed property to check if current user is admin (status = 87)
    private var isAdminUser: Bool {
        let settingsStore = SettingsStore()
        return settingsStore.retrieveUserStatus() == 87
    }
    ...
    var body: some View {
        ZStack {
            ...
            // Conditional view based on user type
            if isAdminUser {
                // Admin view: Show conversation list
                AdminChatConversationListView()
                    .environmentObject(SettingsStore())
            } else {
                // Normal user view: Show chat with support
                VStack(spacing: 0) {
                    // Standardized header
